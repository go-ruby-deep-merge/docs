...
`go-ruby-deep-merge/deep-merge` reimplements Ruby's `deep-merge` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-deep-merge/deep-merge` directly, with no Ruby runtime;
...
Because `deep-merge` is pure Go with cgo disabled, it also compiles to **WebAssembly**
